...
export class ItemService {
  ...
  async getAll(memberId: string): Promise<ItemResponseDto[]> {
  ...
  async getOne(memberId: string, itemId: string): Promise<ItemResponseDto> {
    const couple = await this.membersService.findCoupleByMember(memberId);
    if (!couple) {
      throw new BusinessException(ErrorType.COUPLE_NOT_FOUND);
    }
    ...
    if (!row) {
      throw new BusinessException(ErrorType.ITEM_NOT_FOUND);
    }
  ...
  async purchase(
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      if (existing) {
        throw new BusinessException(ErrorType.ALREADY_OWNED_ITEM);
      }

      if (couple.ecoLovePoint < item.price) {
        throw new BusinessException(ErrorType.NOT_ENOUGH_POINTS);
      }
  ...
  async updatePlacement(
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      for (const dto of itemPlacementDtos) {
        ...
        if (!coupleItem) {
          throw new BusinessException(ErrorType.COUPLE_ITEM_NOT_FOUND);
        }
