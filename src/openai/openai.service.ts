...
import { EcoVerificationType } from '../eco-verification/constant/eco-verification-type.enum';
...
export class OpenaiService {
  ...
  async verifyImageByType(imageUrl: string, type: EcoVerificationType) {
    ...
    try {
      ...
      throw new BusinessException(ErrorCode.VISION_SERVICE_UNAVAILABLE);
    }

    const content = choice?.message?.content;
    if (typeof content !== 'string' || !content.trim()) {
      throw new BusinessException(ErrorCode.INVALID_VISION_RESPONSE);
    }
    ...
    if (!hasValidShape(parsed)) {
      throw new BusinessException(ErrorCode.VISION_SCHEMA_MISMATCH);
    }
